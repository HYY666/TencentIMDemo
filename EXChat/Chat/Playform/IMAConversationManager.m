...
//2.0之前的版本不支持 getConversationList 接口
    ...
    for (TIMConversation *conversation in conversationList)
    {
        ...
        if ([conversation getType] == TIM_GROUP) {//群聊消息
            
            unReadRoom += [conversation getUnReadMessageNum];
            if (unReadRoom != _unReadChatRoomCount) {
                self.unReadChatRoomCount = unReadRoom;
            }
            continue;
        }
        if (conv)
        {
            [conv asyncLoadLocalLastMsg:^{
                if (conv.lastMessage) {
...
//            _chattingConversation.lastMessage = ret.lastMessage;
            return ret;
        }
...
- (void)onNewMessage:(NSArray *)msgs
{
    ...
    for (TIMMessage *msg in msgs)
    {
        ...
        if ([conv getType] == TIM_GROUP) {//不是自己的群组消息就是聊天室
                if ([_chattingConversation type] == TIM_GROUP)
                {
        ...
        if (!updateSucc && _refreshStyle == EIMARefresh_None)
        {
            ...
            else
            {
                ...
                //最后消息附带的头像昵称
                for (int i = 0; i<temp.lastMessage.msg.elemCount; i++) {
                    TIMElem *elem = [temp.lastMessage.msg getElem:i];
                    if ([elem isMemberOfClass:[TIMCustomElem class]]) {
                        ...
                        if (customElem.data) {
                            ...
                            if ([temp.lastMessage.msg isSelf]) {
                                temp.nickName = dic[CUSTOM_MSG_NICK_TO];
                                temp.nickName = temp.nickName.length > 0?temp.nickName:[conv getReceiver];
                                temp.headUrl = dic[CUSTOM_MSG_HEADURL_TO];
                            }else{
                                temp.nickName = dic[CUSTOM_MSG_NICK_FROM];
                                temp.nickName = temp.nickName.length > 0?temp.nickName:[conv getReceiver];
                                temp.headUrl = dic[CUSTOM_MSG_HEADURL_FROM];
                            }
                        }
                    }}
