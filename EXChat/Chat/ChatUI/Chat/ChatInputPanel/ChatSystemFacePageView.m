...
- (void)onClickFace:(UIButton *)btn
{
    ...
    ChatSystemFaceItem *item = [[ChatSystemFaceHelper sharedHelper].systemFaces objectAtIndex:btn.tag-1000 ];
    if (item)
    {
...
- (void)configStart:(NSInteger)index
{
    ...
    while (i < 20)
    {
        ...
        if (index + i < faces.count)
        {
            ...
            btn.tag = item.emojiIndex+1000;
           
//            [btn setBackgroundImage:[item inputGif] forState:UIControlStateNormal];
...
{
    CGSize faceSize = CGSizeMake(35, 35);
