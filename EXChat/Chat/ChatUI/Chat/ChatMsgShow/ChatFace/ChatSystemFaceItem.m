...
- (UIImage *)inputGif
{
     ...
     NSString *name = [NSString stringWithFormat:@"face_icon_0%02d", (int)self.emojiIndex];
...
- (NSString *)gifName
{
    return [NSString stringWithFormat:@"face_icon_0%02d", (int)self.emojiIndex];
...
- (NSString *)getIndexWithTag:(NSString *)tag{
    for (ChatSystemFaceItem *item in _systemFaces) {
        if ([item.emojiTag isEqualToString:tag]) {
            return [NSString stringWithFormat:@"%ld", item.emojiIndex];
        }
    }
    return nil;
}
