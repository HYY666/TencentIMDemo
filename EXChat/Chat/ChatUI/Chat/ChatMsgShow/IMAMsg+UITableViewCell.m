...
- (NSAttributedString *)loadShowChatAttributedText
{
    ...
    NSInteger loc = 0;
    TIMMessage *tempMessage = [[TIMMessage alloc]init];
    BOOL isReplace = NO;
    for (int i = 0; i < self.msg.elemCount; i++)
    {
         TIMElem *e = [self.msg getElem:i];
        if ([e isMemberOfClass:[TIMTextElem class]]) {
            ...
            // 替换匹配的字符串为 searchStr
            NSArray<NSTextCheckingResult *> *result = [regExp matchesInString:textElem.text options:0 range:NSMakeRange(0, textElem.text.length)];
            if (result.count > 0) {
                isReplace = YES;
                NSUInteger tempLoc = 0;
                for (int i = 0; i<result.count; i++) {
                    NSTextCheckingResult *res = result[i];
                   
                    if (tempLoc<res.range.location) {//文本
                        NSRange textRange = NSMakeRange(tempLoc,res.range.location-tempLoc);
                        TIMTextElem *elem = [[TIMTextElem alloc]init];
                        elem.text = [textElem.text substringWithRange:textRange];
                        [tempMessage addElem:elem];
                        tempLoc = res.range.location;
                    }
                        TIMFaceElem *faceElem = [[TIMFaceElem alloc]init];
                        NSString *faceTag = [textElem.text substringWithRange:res.range];
                       faceElem.data = [faceTag dataUsingEncoding:NSUTF8StringEncoding];
                    
                        NSString *faceIndex = [[ChatSystemFaceHelper sharedHelper] getIndexWithTag:faceTag];
                    //+1000
                        if (faceIndex) {
                            faceElem.index = [faceIndex intValue];
                            [tempMessage addElem:faceElem];
                            tempLoc+=res.range.length;
                            if (i == result.count-1) {//文本
                                NSRange textRange = NSMakeRange(tempLoc,textElem.text.length-tempLoc);
                                TIMTextElem *elem = [[TIMTextElem alloc]init];
                                elem.text = [textElem.text substringWithRange:textRange];
                                [tempMessage addElem:elem];
                            }
                            ...
                            TIMTextElem *elem = [[TIMTextElem alloc]init];
                            elem.text = [textElem.text substringWithRange:res.range];
                            [tempMessage addElem:elem];
                            tempLoc = res.range.location;
                ...
                [tempMessage addElem:textElem];
            }
        }else{
            
            [tempMessage addElem:e];
           
        }
    }
    if (!isReplace) {
        tempMessage = self.msg;
    }

    for (int i = 0; i < tempMessage.elemCount; i++) {
         TIMElem *e = [tempMessage getElem:i];
...
- (NSAttributedString *)loadShowLastMsgAttributedText
{
    ...
    {
        TIMElem *e = [self.msg getElem:i];
        if ([e isMemberOfClass:[TIMCustomElem class]]) {//默认发送消息里没有自定义消息
    ...
    if ([self isMultiMsg])
    {
        switch (self.type)
        {
            ...
            case EIMAMSG_Sound:
                ...
                break;
            case EIMAMSG_Gift:
                return [GiftChatTableViewCell class];
            default:
...
//    UIEdgeInsetsMake(<#CGFloat top#>, <#CGFloat left#>, <#CGFloat bottom#>, <#CGFloat right#>)
    if (self.isMineMsg)
    {
        return UIEdgeInsetsMake(kDefaultMargin/2+3, kDefaultMargin/2+2, kDefaultMargin/2+3, kDefaultMargin + 2);
    }
    else
    {
        return UIEdgeInsetsMake(kDefaultMargin/2+3, kDefaultMargin + 2, kDefaultMargin/2+3, kDefaultMargin/2 + 2);
    }
}
...
- (CGSize)contentSizeInWidth:(CGFloat)width
{
    ...
    CGFloat h = 0;
    CGFloat w = 0;
    TIMElem *elem = [self.msg getElem:0];
    if (_type == EIMAMSG_Gift) {
        return CGSizeMake(GIFT_CELL_WIDTH, GIFT_CELL_HEIGHT);
    }
    
    if ([self isMultiMsg]&&![elem isMemberOfClass:[TIMSoundElem class]])
    {//语音消息特殊处理
