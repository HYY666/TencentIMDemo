...
- (void)setDraftMsg:(IMAMsg *)draft
{
    ...
    if (draft)
    {
        for (int index = 0; index < draft.msgDraft.elemCount; index++)
        {
            ...
            {
                NSDictionary *attributeDict = [NSDictionary dictionaryWithObjectsAndKeys:kAppLargeTextFont,NSFontAttributeName,nil];
                NSAttributedString *att = [[NSAttributedString alloc] initWithString:((TIMTextElem *)elem).text attributes:attributeDict];
