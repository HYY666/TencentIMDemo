...
@interface TIMElemBaseTableViewCell ()
...
- (void)configWith:(IMAMsg *)msg
{
    ...
    NSString *title = @"";
    NSString *image = @"";
    for (int i = 0; i < _msg.msg.elemCount; i++) {
        TIMElem *elem = [_msg.msg getElem:i];
        if ([elem isMemberOfClass:[TIMCustomElem class]]) {
            ...
            if (customElem.data) {
                ...
                if ([_msg.msg isSelf]) {
                    image = dic[CUSTOM_MSG_HEADURL_FROM]?dic[CUSTOM_MSG_HEADURL_FROM]:[UserHelper sharedHelper].user.userImage;
                    
                    title = dic[CUSTOM_MSG_NICK_FROM]?dic[CUSTOM_MSG_NICK_FROM]:[UserHelper sharedHelper].user.userNickname;
                }else{
                    image = dic[CUSTOM_MSG_HEADURL_FROM];
                    title = dic[CUSTOM_MSG_NICK_FROM];
                }
                if ([dic[CUSTOM_SYSTEM_MESSAGE] intValue] == 2) {
                    self.type = dic[CUSTOM_SYSTEM_MESSAGE];
                    self.param = dic[CUSTOM_MSG_PARAM];
      ...
      [_icon sd_setImageWithURL:[NSURL URLWithString:image] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:DEFAULT_AVATAR]];
