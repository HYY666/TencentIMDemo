...
//        self.navigationController.interactivePopGestureRecognizer.enabled = YES;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onUpdateInputStatus:) name:kUserInputStatus object:nil];
     ...
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(sendGiftMessageHandle:) name:sendGiftMessage object:nil];
...
- (void)sendGiftMessageHandle:(NSNotification *)sender{
    
    if ([sender.userInfo[@"userId"] isEqualToString:self.userId]) {
        NSArray *msgList =sender.userInfo[@"msg"];
        [self showMsgs:msgList];
    }
...
{
    if ([[UserHelper sharedHelper].user.userLevel isEqualToString:@"0"]) {
        if ([[DataSource shareInstance].userChatFreeNumber  intValue] < 1) {
            [EMAlertView showAlertWithTitle:@"提示框" message:@"系统赠送聊天额度已消费,当前等级不够,无法继续发言,请升级." completionBlock:^(NSUInteger buttonIndex, EMAlertView *alertView) {
                if (buttonIndex == 1) {
//                    [self.navigationController pushViewController:[RechargeViewController new] animated:YES];
                }
            } cancelButtonTitle:@"确定" otherButtonTitles:@"去充值"];
            return;
        }
    }
    if (msg)
...
//        _isSendMsg = YES;
        [_tableView beginUpdates];
        __weak ChatViewController *ws = self;
        NSMutableDictionary *dic = [ParameterHelper getCustomDataWithToNick:self.nickName toHeadUrl:self.headUrl type:@"0" param:nil];
        TIMTextElem *textElem = [[TIMTextElem alloc]init];
        if (msg.msg.elemCount >= 1) {
            for (int i = 0; i < msg.msg.elemCount; i++) {
                TIMElem *elem = [msg.msg getElem:i];
                if ([elem isKindOfClass:[TIMTextElem class]]) {
                    TIMTextElem *tempElem = (TIMTextElem *)elem;
                    textElem.text = [NSString stringWithFormat:@"%@%@",textElem.text?textElem.text:@"",tempElem.text];
                }
                if ([elem isKindOfClass:[TIMFaceElem class]]) {
                    TIMFaceElem *tempElem = (TIMFaceElem *)elem;
                    ChatSystemFaceItem *item =  [[ChatSystemFaceHelper sharedHelper]emojiItemOf:tempElem.index];
                    textElem.text = [NSString stringWithFormat:@"%@%@",textElem.text?textElem.text:@"",item.emojiTag];
                }
            }
        }
        if (textElem.text.length > 0) {
            TIMMessage *tempMsg = [[TIMMessage alloc]init];
            [tempMsg addElem:textElem];
            msg.msg = tempMsg;
        }
        
        if (!msg.msg) {
            msg.msg = [[TIMMessage alloc]init];
        }
      ...
      NSArray *newaddMsgs = [_conversation sendMessage:msg completion:^(NSArray *imamsglist, BOOL succ, int code) {
          ...
          if (succ) {
              if ([[UserHelper sharedHelper].user.userLevel isEqualToString:@"0"]) {
                  int chatFreeNumber = [[DataSource shareInstance].userChatFreeNumber  intValue] -1;
                  [DataSource shareInstance].userChatFreeNumber = [NSString stringWithFormat:@"%d",chatFreeNumber];
                  [[NSUserDefaults standardUserDefaults]setObject:[DataSource shareInstance].userChatFreeNumber forKey:userChatFreeNumberKey];
              }
            ...
            if (!succ)
            {
                ...
                {
                    [HYYHudView showMsg:@"含有敏感词，未发送成功!" inView:nil];
                    return;
//                    IMAMsg *msg = [IMAMsg msgWithCustom:EIMAMSG_SaftyTip param:@"含有敏感词，未发送成功!"];
