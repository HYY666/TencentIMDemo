...
- (void)asyncLoadLocalLastMsg:(CommonVoidBlock)block
{
    ...
    for (TIMMessage *msg in msgs)
    {
        ...
        if (imamsg.msg.elemCount < 2&&![user.userId isEqualToString:[DataSource shareInstance].imKefuId]) {//   不是客服 消息体小于2代表没头像昵称不处理
            [imamsg.msg remove];//移除
            [imamsg.msg delFromStorage];//从本地列表移除
            continue;
        }
        ...
        if (imamsg)
        {
            ...
            //最后消息附带的头像昵称
            for (int i = 0; i<_lastMessage.msg.elemCount; i++) {
                TIMElem *elem = [_lastMessage.msg getElem:i];
                if ([elem isMemberOfClass:[TIMCustomElem class]]) {
                    TIMCustomElem *customElem =(TIMCustomElem *)elem;
                    if (customElem.data) {
                        NSDictionary *dic =[NSJSONSerialization JSONObjectWithData:customElem.data options:NSJSONReadingMutableLeaves error:nil];
                        
                        if ([_lastMessage.msg isSelf]) {
                            self.nickName = dic[CUSTOM_MSG_NICK_TO];
                            self.nickName = self.nickName.length > 0?self.nickName:[_conversation getReceiver];
                          self.headUrl = dic[CUSTOM_MSG_HEADURL_TO];
                        }else{
                            self.nickName = dic[CUSTOM_MSG_NICK_FROM];
                             self.nickName = self.nickName.length > 0?self.nickName:[_conversation getReceiver];
                            self.headUrl = dic[CUSTOM_MSG_HEADURL_FROM];
                        }
                    }
                }}
...
- (void)asyncLoadRecentMessage:(NSInteger)count from:(IMAMsg *)msg completion:(HandleMsgBlock)block
{
    ...
    [_conversation getMessage:(int)count last:msg.msg succ:^(NSArray *array) {
        ...
        for (TIMMessage *msg in array)
        {
            if (msg.elemCount < 2&&![[msg sender] isEqualToString:[DataSource shareInstance].imKefuId]) {//   不是客服 消息体小于2代表没头像昵称不处理
                [msg remove];//移除
                [msg delFromStorage];//从本地列表移除
                
                [arr removeObject:msg];
            }}
        
        NSArray *recentIMAMsg = [ws onLoadRecentMessageSucc:arr];
        if (block)
        {
...
//    if (msg.msg.elemCount < 2&&![user.userId isEqualToString:[DataSource shareInstance].imKefuId]) {//   不是客服 消息体小于2代表没头像昵称不处理
//        return;
//    }
//    //获取新消息 剔除视频底层消息
//    TIMElem *elem = [msg.msg getElem:msg.msg.elemCount-1];
//    
//    if ([elem isMemberOfClass:[TIMCustomElem class]]) {
//        TIMCustomElem *customElem =(TIMCustomElem *)elem;
//        if (customElem.data) {
//            NSDictionary *dic =[NSJSONSerialization JSONObjectWithData:customElem.data options:NSJSONReadingMutableLeaves error:nil];
//            if ([dic[CUSTOM_SYSTEM_MESSAGE] isEqualToString:@"1"]) {//
//                
//                return;
//            }
//
