...
- (MBProgressHUD *)loadingShowCustom:(NSString *)msg inView:(UIView *)view{
    UIView *inView = view ? view : [BaseAppDelegate sharedAppDelegate].window;
    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:inView];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (![NSString isEmpty:msg])
        {
            ...
            hud.labelFont = FONT_12;
        }
        //创建一个可变数组
        NSMutableArray *ary=[NSMutableArray new];
        for(int i=0;i<15;i++){
            //通过for 循环,把我所有的 图片存到数组里面
            NSString *imageName=[NSString stringWithFormat:@"call_load_%d.png",i];
...
//        UIImageView *imageView = [[UIImageView alloc]init];
        ...
        // 给图片控件添加图片对象
        [imageView setImage:[UIImage imageNamed:@"call_load_0"]];
